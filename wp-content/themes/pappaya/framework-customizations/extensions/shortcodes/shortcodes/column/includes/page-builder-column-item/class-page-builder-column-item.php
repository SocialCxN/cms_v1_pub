...
class Page_Builder_Column_Item extends Page_Builder_Item {
	...
	private function get_item_data() {
		...
		$data['l10n'] = array(
			'edit' => esc_html__( 'Edit', 'pappaya' ),
			'duplicate' => esc_html__( 'Duplicate', 'pappaya' ),
			'remove' => esc_html__( 'Remove', 'pappaya' ),
			'title' => esc_html__( 'Column', 'pappaya' ),
		);
	...
	protected function get_thumbnails_data() {
		...
		foreach ( $builder_widths as $key => $value ) {
			$column_thumbnails[ $key ] = array(
				'tab'         => esc_html__( 'Layout Elements', 'pappaya' ),
				...
				'description' => apply_filters( 'fw_ext_shortcodes_column_description',
					sprintf( esc_html__( 'Add a %s column', 'pappaya' ), $value['title'] ), $key ),
